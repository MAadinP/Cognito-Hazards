...
sword_idle = {
    1: load_animation_frames(os.path.join(SWORD_PATH, "sword_idle"), scale_size=(300, 300)),
    2: load_animation_frames(os.path.join(SWORD_PATH, "sword_idle"), flip=True, scale_size=(300, 300))
}
sword_combo = {
    1: load_animation_frames(os.path.join(SWORD_PATH, "sword_combo"), scale_size=(300, 300)),
    2: load_animation_frames(os.path.join(SWORD_PATH, "sword_combo"), flip=True, scale_size=(300, 300))
}
...
def request_play_again():
    ...
    if player_id is not None:
        client_socket.sendto(f"PLAY_AGAIN,{player_id}".encode(), (SERVER_IP, SERVER_PORT))
    ...
    while True:
        try:
            ...
            if decoded_data.startswith("GAME_START"):
                ...
                pygame.event.post(pygame.event.Event(pygame.USEREVENT, {"type": "game_reset"}))
...
def draw_boss():
    ...
    if (current_animation == "take_hit" and now - hit_timer > 500) or (current_animation == "cleave" and now - hit_timer > 700):
        current_animation = "idle"
        frame_index = 0  # Reset frame index when switching to idle

    elif current_animation == "die" and frame_index == len(animation_frames[current_animation]) - 1:
    ...
    if now - last_update > ANIMATION_SPEED:
        if current_animation == "idle":
            ...
            frame_index = min(frame_index + 1, len(animation_frames[current_animation]) - 1)
        last_update = now
