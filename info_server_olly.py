...
def send_to_all_tcp_clients(message):
    """Send a message to all connected TCP clients."""
    message_encoded = message.encode("utf-8")  # Encode the message before sending

    for client in TCP_clients:
        try:
            client.sendall(message_encoded)  # Send data
            print(f"Sent to TCP client: {message}")  # Debugging output
        except Exception as e:
            print(f"Error sending to TCP client: {e}")
            TCP_clients.remove(client)  # Remove disconnected clients

def game_state_watcher():
    global game_running
    last_state = game_running  # Store initial state

    while True:
        if game_running != last_state:  
            if last_state:
                send_on_jtag("on")
            else:
                send_on_jtag("off")
            last_state = game_running  
        time.sleep(0.1)  

threading.Thread(target=game_state_watcher, daemon=True).start()
